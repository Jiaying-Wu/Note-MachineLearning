...
* 2.[XGBoost](https://xgboost.readthedocs.io/en/latest/)
    ...
    * 2.1 The regularization term in XGBoost
    
    * 2.2

* 3.[LightGBM](https://lightgbm.readthedocs.io/en/latest/)

    * 3.1 The regularization term in LightGBM
    
    * 3.2

* 4.[CatBoost](https://catboost.ai)

    * 4.1 The regularization term in CatBoost
...
$$
\sum^{L}_{l = 1} ({Residual}_{\text{{t, d, left}}} - \overline{{Residual}_{\text{{t, d, Left}}}})^2 + \sum^{R}_{r = 1} ({Residual}_{\text{{t, d, right}}} - \overline{{Residual}_{\text{{t, d, Right}}}})^2 \leq \sum^{N}_{n = 1} ({Residual}_{\text{{t, d, node}}} - \overline{{Residual}_{\text{{t, d, Node}}}})^2 \\
...
$$
\sum^{L}_{l = 1} ({Residual}_{\text{{t, d, left}}} - \overline{{Residual}_{\text{{t, d, Left}}}})^2 + \Omega_{\text{{t, (d+1), Left}}} +  \sum^{R}_{r = 1} ({Residual}_{\text{{t, d, right}}} - \overline{{Residual}_{\text{{t, d, Right}}}})^2 + \Omega_{\text{{t, (d+1), Right}}}
$$
...
$$
\sum^{L}_{l = 1} ({Residual}_{\text{{t, d, left}}} - \overline{{Residual}_{\text{{t, d, Left}}}})^2 + \Omega_{\text{{t, (d+1), Left}}} + \sum^{R}_{r = 1} ({Residual}_{\text{{t, d, right}}} - \overline{{Residual}_{\text{{t, d, Right}}}})^2 + \Omega_{\text{{t, (d+1), Right}}} \\
< \sum^{N}_{n = 1} ({Residual}_{\text{{t, d, node}}} - \overline{{Residual}_{\text{{t, d, Node}}}})^2 + \Omega_{\text{{t, d, Node}}} \\
...
In order to avoid overfitting, we will set the hard threshold to limit the depth and/or end leaves of each tree in GBDT.

**1.Limit the depth in each tree**

For a relatived large sample, we can set the maximum depths of each tree as 5.

Then, the maximum end leaves of each tree is $2^d = 2^5 = 32$. 

Each end leaf associated with at most interaction of 5 variables. Since we search thorught all the variavles in each split.

For example, we can split the obervation into leaf {1, A} and leaf {1, B} base on {is age younger than 30?} in depth 1. Then, leaf {1, B} split into leaf {2, C} and leaf {2, D} base on {is age younger than 60?} in depth 2.
...
**Limit the depth can prevent the model grown a large (deep) tree, large tree will suffer form large variance in out of sample prediction.**

Also, less 

![](https://openstax.org/resources/ac97e737d79567167c2776305b494f01009d948a)

(Image 4 from openstax)


**2.Limit the end leaves in each tree**
...
### 2.![](https://upload.wikimedia.org/wikipedia/commons/6/69/XGBoost_logo.png){width="15%" height="15%"}
...
### 3.![](https://cambridgespark.com/wp-content/uploads/2018/10/lightGBM-logo.png){width="18%" height="18%"}
...
### 4.![](https://camo.githubusercontent.com/1ba204e6a09e6f13c919dcf961fe5a9a7f2d6e30/687474703a2f2f73746f726167652e6d64732e79616e6465782e6e65742f6765742d646576746f6f6c732d6f70656e736f757263652f3235303835342f636174626f6f73742d6c6f676f2e706e67){width="25%" height="25%"}
